    ...
    window.FilterWizard = class FilterWizard {
        ...
        getStep1Content() {
            if (this.state.loading) {
                return `
                <h2 class="text-xl font-bold mb-4">📋 Select Input File</h2>
                <div class="space-y-2">
            ...
            const isRaw = this.state.mode === 'raw';
            ...
            if (isRaw) {
                ...
                files = (this.state.lists || []).filter(list =>
            ...
            <div class="tabs tabs-boxed mb-4 bg-base-200 p-1">
                <a class="tab ${!isRaw ? 'tab-active' : ''} flex-1" onclick="window.currentWizard?.setMode('clean')">
                    ✨ Clean Lists
                </a>
                <a class="tab ${isRaw ? 'tab-active' : ''} flex-1" onclick="window.currentWizard?.setMode('raw')">
                    📄 Raw Input Files
                </a>
            </div>

            <p class="text-slate-600 dark:text-slate-400 mb-4">
                ${isRaw ? 'Select a raw file (LVP/TXT) to process from scratch' : 'Choose a pre-cleaned email list to apply the filter to'}
            </p>

            ${files.length === 0 ? `
                <div class="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                    <div class="font-medium text-yellow-900 dark:text-yellow-400">⚠️ No files found</div>
                    <div class="text-sm text-yellow-800 dark:text-yellow-300">
                        ${isRaw ? 'No files found in input/ directory.' : 'Process some email lists first to create clean files.'}
                    </div>
                </div>
            ` : `
                ...
                const isSelected = this.state.selectedList && (this.state.selectedList.filename === filename || this.state.selectedList === filename);

                return `
                        <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-base-200 transition-colors ${isSelected ? 'border-primary bg-base-200' : ''}">
                            <input type="radio" name="list" value="${filename}" class="w-4 h-4" ${isSelected ? 'checked' : ''}
                                   onchange="window.currentWizard?.onListSelect('${filename}')">
                            <div class="flex-1">
                                <div class="font-medium">${displayName}</div>
                                ${!isRaw && file.total_emails ? `
                                    <div class="text-sm text-slate-500">
                                        ${file.country ? `🌍 ${file.country}` : ''}
                                        ${file.category ? `• ${file.category}` : ''}
                                        • ${file.total_emails.toLocaleString()} emails
                                    </div>
                                ` : ''}
                                ${isRaw ? `
                                    <div class="text-sm text-slate-500">
                                        ${filename.endsWith('.lvp') ? 'LVP File' : 'Text File'}
                                    </div>
                                ` : ''}
                            </div>
                            ${(file.file_type === 'lvp' || filename.endsWith('.lvp')) ? '<span class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-2 py-1 rounded">LVP</span>' : ''}
                        </label>
                    `}).join('')}
                </div>
            `}
        ...
        getStep3Content() {
            ...
            <div class="space-y-6">
                ...
                <!-- Raw File Options (Only in Raw Mode) -->
                ${this.state.mode === 'raw' ? `
                <div class="border-b pb-4">
                    <h3 class="font-medium mb-3">⚙️ Processing Options</h3>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium mb-2">Score Threshold</label>
                            <div class="flex items-center gap-4">
                                <input type="number" id="wizard-score-threshold" value="30" min="10" max="100" step="0.1"
                                       class="w-24 px-3 py-2 border border-base-300 rounded-lg bg-base-100 text-base-content">
                                <span class="text-sm text-slate-500">Minimum score for final CLEAN list (recommended: 30 for LVP)</span>
                            </div>
                        </div>

                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" id="wizard-skip-base" class="checkbox checkbox-primary checkbox-sm">
                            <div>
                                <div class="font-medium text-sm">Skip Base Filtering</div>
                                <div class="text-xs text-slate-500">Check this if the file was already processed through check-lvp</div>
                            </div>
                        </label>
                    </div>
                </div>
        ...
        getStep5Content() {
            ...
            <div class="space-y-3">
                <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-base-200">
                    ...
                    <div>
                        <div class="font-medium">${this.state.mode === 'raw' ? '🚀 Start Full Workflow' : '▶️ Apply Filter Now'}</div>
                        <div class="text-sm text-slate-500">${this.state.mode === 'raw' ? 'Process raw file through Base Filter + Smart Filter' : 'Process selected clean list immediately'}</div>
                    </div>
                </label>
        ...
        async finish() {
            ...
            if (this.onComplete) {
                this.onComplete({
                    ...
                    // Extra options for raw mode
                    scoreThreshold: document.getElementById('wizard-score-threshold')?.value,
                    skipBaseFiltering: document.getElementById('wizard-skip-base')?.checked
                });
            }
        ...
        /**
         * Load raw files from API
         */
        async loadRawFiles() {
            if (this.cache.rawFiles) {
                this.state.rawFiles = this.cache.rawFiles;
                return;
            }

            try {
                const response = await window.api.get('/api/all-files');
                const files = response.data.input_files || [];

                this.cache.rawFiles = files;
                this.state.rawFiles = files;
                console.log(`Loaded ${files.length} raw files`);
            } catch (error) {
                console.error('Failed to load raw files:', error);
                // Don't block init, just log error
            }
        }

        /**
         * Set wizard mode (clean vs raw)
         */
        setMode(mode) {
            if (this.state.mode !== mode) {
                this.state.mode = mode;
                this.state.selectedList = null; // Reset selection
                this.render();
            }
        }
        ...
        onListSelect(filename) {
            if (this.state.mode === 'raw') {
                // For raw files, we just store the filename string or object if available
                this.state.selectedList = { filename: filename, display_name: filename };
            } else {
                // For clean lists, find the full object
