"""
Smart Filter for Czech Powder Metallurgy Industry
==================================================

Фильтр для чешского рынка порошковой металлургии.
Работает на чешском и английском языках.

Target: B2B компании в Czech Republic специализирующиеся на:
- Порошковой металлургии (prášková metalurgie)
...
class CzechPMHardExclusionFilter:
    """
...
class CzechPMLeadScorer:
    ...
    - Geographic Priority (30%): Близость к Czech Republic
...
def filter_czech_pm_contacts(emails_data: List[Dict], config_path: Optional[str] = None) -> Dict:
    """
    Основная функция фильтрации Czech PM контактов
    ...
    if config_path is None:
        ...
        config_path = config_dir / "czech_powder_metal.json"

    with open(config_path, 'r', encoding='utf-8') as f:
        config = json.load(f)

    # Инициализируем фильтры
    hard_filter = CzechPMHardExclusionFilter(config)
    detector = CzechPMDetector(config)
    classifier = CzechPMClassifier(config)
    scorer = CzechPMLeadScorer(config)
