#!/usr/bin/env python3
"""
Simple test without external dependencies - uses only urllib (built-in)
"""
import json
from urllib.request import Request, urlopen
from urllib.error import HTTPError, URLError

def test_endpoint():
    """Test the /api/smart-filter/apply endpoint using urllib"""

    url = "http://localhost:8089/api/smart-filter/apply"

    # Test config
    test_config = {
        "config": {
            "metadata": {
                "name": "Test Filter",
                "version": "1.0.0",
                "description": "Test filter configuration"
            },
            "target": {
                "country": "IT",
                "industry": "hydraulics"
            },
            "scoring": {
                "weights": {
                    "email_quality": 0.10,
                    "company_relevance": 0.45,
                    "geographic_priority": 0.30,
                    "engagement": 0.15
                },
                "thresholds": {
                    "high_priority": 100,
                    "medium_priority": 50,
                    "low_priority": 10
                }
            },
            "company_keywords": {
                "positive": ["hydraulics", "pumps"],
                "negative": ["alibaba", "marketplace"]
            }
        },
        "timestamp": "2025-10-30T12:00:00Z"
    }

    print("Testing /api/smart-filter/apply endpoint...")
    print(f"URL: {url}")
    print("-" * 50)

    try:
        # Prepare request
        data = json.dumps(test_config).encode('utf-8')
        req = Request(url, data=data, headers={'Content-Type': 'application/json'})

        # Send request
        with urlopen(req, timeout=10) as response:
            status_code = response.status
            response_data = response.read().decode('utf-8')

        print(f"Status Code: {status_code}")
        print(f"Response:")
        print(json.dumps(json.loads(response_data), indent=2, ensure_ascii=False))

        if status_code == 200:
            print("\n✅ SUCCESS! Endpoint is working correctly.")
        else:
            print(f"\n⚠️ WARNING: Status code {status_code}")

    except HTTPError as e:
        print(f"❌ HTTP ERROR: {e.code} {e.reason}")
        try:
            error_body = e.read().decode('utf-8')
            print(f"Error details:\n{error_body}")
        except:
            pass

    except URLError as e:
        print("❌ CONNECTION ERROR: Cannot connect to server.")
        print(f"   Reason: {e.reason}")
        print("   Make sure web_server.py is running:")
        print("   python web_server.py")

    except Exception as e:
        print(f"❌ ERROR: {e}")

if __name__ == "__main__":
    test_endpoint()
